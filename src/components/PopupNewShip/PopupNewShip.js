...
export default function PopupNewShip({ onClose, iceClasses, getIceClasses, changeOption, setIceClass, setNewShip }) {
        ...
        >
            <form className="form form-ship" name="form_ship" onSubmit={onSubmit}>
                <div className="form__field">
                    <label className={`form__label ${shipName === "" ? "":"form__label_active"}`} htmlFor="ship-name">Название судна</label>
                    <input
                    id="ship-name"
                    name="ship-name"
                    className={`form__input ${shipName === "" ? "":"form__input_active"}`}
                    type="text"
                    ...
                    required
                    />
                    {isErrorName && <span className="form__input-error">{errorsShips.name || ""}</span>}
                </div>
                ...
                <div className="form__field">
                    <label className={`form__label ${shipSpeed === "" ? "":"form__label_active"}`} htmlFor="ship-speed">Скорость, узлы (по чистой воде)</label>
                    <input
                    name="ship-speed"
                    id="ship-speed"
                    className={`form__input ${shipSpeed === "" ? "":"form__input_active"}`}
                    type="text"
                    ...
                    required
                    />
                    {isErrorSpeed && <span className="form__input-error">{errorsShips.speed || ""}</span>}
                </div>

                <div className="form__btns">
                    <button className="form__btn content__btn_secondary" type="button" onClick={onClose}>Отменить</button>
                    <button className={`form__btn content__btn ${isDisabled ? "content__btn_disabled":""}`} type="submit" disabled={isDisabled}>Добавить</button>
                </div>
            </form>
        </Popup>
    );
