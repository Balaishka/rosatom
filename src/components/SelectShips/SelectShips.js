...
function SelectShips({ options, selectedOption, changeOption, name, setSelected, myClass, clue, setInfoShip, children }) {
        ...
        <div className={`my-select ${myClass}`}>
            ...
            <ul className={`my-select__list ${isOpen ? "my-select__list_opened":""}`}>
                {options.map((option, index) => {
                    ...
                    return (
                        <li key={index} className={`my-select__item my-select__item_type_ships ${selectedOption === index ? "my-select__item_active":""}`} onClick={handleOption}>
                            {option.name}
